 ...
 * 
 * Simple system for sliding notifications.
 */
export const ToastContainer: React.FC<{ toasts: Toast[] }> = ({ toasts }) => {
  return (
    <div className="ads-toast-container" style={{
      position: 'fixed',
      bottom: '1rem',
      right: '1rem',
      display: 'flex',
      ...
      {toasts.map(toast => (
        <div 
          key={toast.id} 
          className={`ads-toast ads-toast--${toast.type || 'info'}`}
          style={{
            padding: '0.75rem 1.5rem',
            fontFamily: 'var(--ads-font-mono)',
            textTransform: 'uppercase',
            fontSize: '0.8rem',
            animation: 'ads-slide-in 0.3s ease-out'
          }}
        >
          {toast.message}
        </div>
